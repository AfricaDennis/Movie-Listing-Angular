...
export class ProducerFormComponent implements OnInit{
  ...
  constructor(private location: Location, private route: ActivatedRoute, private producerService: ProducerService) { 
  ...
  save(){
    ...
    if (!this.isNewContext) {
      ...
      this.producerService.updateProducer(request, this.id).subscribe(() => this.goBack());
    }
    else{
      let request = {
        ...this.producerForm.getRawValue()
